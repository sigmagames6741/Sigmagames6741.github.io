...
// Apply settings with performance considerations
function applySettings() {
    ...
    if (settings.optimization === 'performance') {
...
const MAX_VISIBLE_CARDS = 50; // Limit visible cards for performance

// Initialize lazy loading
function initLazyLoading() {
    // Fallback for browsers without IntersectionObserver
    if (!('IntersectionObserver' in window)) {
        console.log('IntersectionObserver not supported, showing all cards');
    ...
    gameObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            ...
            if (entry.isIntersecting) {
                // Card is entering viewport - make it visible
                showCard(card);
                visibleCards.add(card);

                // Limit visible cards for performance
                if (visibleCards.size > MAX_VISIBLE_CARDS) {
                    // Hide the oldest visible card
                    const oldestCard = visibleCards.values().next().value;
                    if (oldestCard && oldestCard !== card) {
                        hideCard(oldestCard);
                        visibleCards.delete(oldestCard);
                    }
                }
            } else {
                // Card is leaving viewport - hide it to save memory
                hideCard(card);
...
// Render game library with lazy loading
function renderGameLibrary(filter = '') {
    ...
    filteredGames.forEach((game, index) => {
        ...
        card.className = 'game-card visible'; // Make cards visible by default
    ...
    // No lazy loading - games are visible immediately
}

// Load a game with performance optimization
function loadGame(game) {
    if (currentGame === game) return; // Prevent unnecessary reloads
    ...
    content.innerHTML = '<div style="text-align: center; padding: 2rem;">Loading game...</div>';
...
// Function to show games from a specific category
